...
if __name__ == '__main__':   
    ...
    startingwell = "A1"
    displacement = -float(input("What is the distance between the center of each well?\n"))
    ...
    for col in range(cols):
        ...
        img = cv.circle(img,(int(cposx),int(cposy)),40,(255,255,255),-1)
        img = cv.putText(img,names[row,col],org = (int(cposx-20),int(cposy+15)), fontFace=cv.FONT_HERSHEY_SIMPLEX,fontScale=1,color = (0,255,0),thickness=2,lineType=cv.LINE_AA)
